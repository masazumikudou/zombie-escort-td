...
| **1-4.5クローズ後に着手可・P3開始前必須** | **R-6: ステージJSON検証スクリプト新設** | `validate_stage.js`（新規） | ✅ 完了（2026-08-11。3便構成で実装。1便目=座標・ルール突き合わせ9項目、2便目=枠外spawn侵入余裕チェック(項目10)＋`--margin`猶予マップ出力、3便目=R-6原文の残り3項目（項目11=buildSpots vs propフットプリント衝突・項目12=spawnキー未定義参照検出・項目13=leashTo歩行可能性）。下記R-6セクション参照） |
...
**実績**: 片側修正事故が既に4回発生（Phaserスタブ漏れによるブラウザクラッシュ／vm束縛バグ／escorts[0]のみ対応のリグレッション／road-only制約未実装）。リスク登記簿7番の実体。
...
**【新発見・2026-07-21】ground_cellsのroad-only制約がシム未実装** ✅ 対応済み（下記参照）

GameScene.jsはground_cellsを1件でも指定すると「登録セル以外はゾンビ移動禁止」の制約を発動するが、run_sim.js・simulator.htmlにはこのロジックが存在しない。「シムでは正常に見えるのに実機だけゾンビが護衛にたどり着けず詰む」事故が起こりうる状態。「シム=実機の完全一致」の鉄則に反する乖離。

発覚経緯: 1-4bのYテストステージでground_cellsがescort本道・Y地点を含まない状態のまま組んでしまい、実機のみ詰む状態になっていた（シム上は問題なく見えた）。

対策: ①シム側にroad-only制約を実装して一致させる／②制約自体を廃止しbuildSpots/obstaclesのみで統制する、の2案を検討。**②（廃止）は選ばず、①を採用**——ground_cells未指定時は現行通り全域移動可能のまま、指定時のみ制約発動という既存のopt-in仕様を維持しつつ、シム側にも同一ロジックを実装する形で解消（run_sim.js/simulator.html両方に実装・既存ステージで回帰確認済み）。
...
## R-6: ステージJSON検証スクリプト新設【✅完了・2026-08-11】
...
1. buildSpots vs propフットプリント衝突（今回発見した件）。**【✅2026-08-11・validate_stage.js項目11で実装】** シム実行前の静的チェックとして実装（`propFootprintCells`を流用）。run_sim.js/simulator.htmlのランタイムゲート（2026-07-27実装分）と役割は別で、両方が併存する
2. buildSpots vs 道路セル(ground_cells)衝突（旧`(5,8)`の件と同種）。**【✅2026-08-11・validate_stage.js項目6で実装】**
3. initial/triggersの`spawn`キーが`spawns`に未定義の場合、黙殺せず即エラー。**【✅2026-08-11・validate_stage.js項目12で実装】**
4. スポーン座標・leashTo座標の歩行可能性（`ground_cells`定義時のroad-only判定と整合）。**【✅2026-08-11】** スポーン座標は項目1、leashTo座標は項目13で実装
5. トリガーの時間整合性——発火時点の護衛位置から敵の到達時刻を逆算し、護衛がすでに通過済み（間に合わない）なら警告。**【✅2026-08-11・validate_stage.js項目10で実装】** 枠外spawnの侵入所要時間とsegment猶予を比較しERROR/WARN判定。加えて`--margin`オプションで全escort×全waypointの猶予一覧を出力する補助モードも実装
6. **【2026-07-26追加】enemyのhp明示指定にコメント理由が無い場合の警告** — ステージJSONの`enemy`にhpが明示指定されていて（balance.json値を上書きしてしまう仕様）、かつ`_comment`による理由の記載が無い場合に警告を出す。balance.jsonでバランス調整しても反映されない事故（spec06 §9地雷14）の再発防止。**【✅2026-08-11・validate_stage.js項目9で実装】**

**実装結果**: 3便構成（コミットe149600・cebd8d0・cb9b9e1）。項目1〜9=座標・ルール突き合わせ（1便目）、項目10=トリガー時間整合性＋`--margin`猶予マップ（2便目）、項目11〜13=本セクション原文の残り3項目（3便目）。確定ABAステージ（stage_確定ABAステージ.json）で全項目OKを確認済み。各項目とも意図的に壊したテストJSONでERROR/WARNが正しく出ることを確認済み。

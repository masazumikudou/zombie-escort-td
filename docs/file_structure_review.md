...
6. **【2026-07-26追加】enemyのhp明示指定にコメント理由が無い場合の警告** — ステージJSONの`enemy`にhpが明示指定されていて（balance.json値を上書きしてしまう仕様）、かつ`_comment`による理由の記載が無い場合に警告を出す。balance.jsonでバランス調整しても反映されない事故（spec06 §9地雷14）の再発防止

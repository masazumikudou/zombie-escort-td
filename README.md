...
### プロップスプライト（複数セル占有の静的オブジェクト）
```
assets/sprites/prop/{type}.png
  type : shop | （将来追加される種別）
```
プロップの `cols × rows` セル全体が A* のブロックとして自動展開される。  
描画深度はマップ（depth 0）の上、キャラクター（depth 3）の下（depth 2）。
